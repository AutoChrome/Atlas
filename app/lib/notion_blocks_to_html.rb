...
# bulleted/numbered lists, quote, code, divider, to_do, image, callout
...
# a simple table. Anything else (child pages, embeds, synced blocks,
# columns, equations, files/video/audio, ...) is silently skipped rather
# than raising — a Notion page throwing an unexpected block type at this
# should degrade gracefully, not break the whole sync.
class NotionBlocksToHtml
  ...
  private
    # Consecutive bulleted_list_item / numbered_list_item blocks need to
    # share ONE <ul>/<ol>, not get one each — this groups runs of the same
    # list type together first; everything else passes through as a
    # single-item group of its own.
    ...
    def render_list(items)
      tag = items.first["type"] == "bulleted_list_item" ? "ul" : "ol"
    ...
    def render_block(block)
      ...
      case type
      when "paragraph" then "<p>#{render_rich_text(data["rich_text"])}</p>"
      ...
      when "to_do" then render_to_do(data)
      when "image" then render_image(data)
      when "callout" then render_callout(data)
      when "toggle" then "<p><strong>#{render_rich_text(data["rich_text"])}</strong></p>#{render_children(block)}"
    ...
    def render_to_do(data)
      mark = data["checked"] ? "☑" : "☐"
      "<p>#{mark} #{render_rich_text(data["rich_text"])}</p>"
    end
    ...
    def render_table(block)
      ...
      row_html = rows.each_with_index.map { |row, index|
